...
import com.adnakiwoch.platform.streaming_api.service.vid.VidStoreService;
...
public class GoStreamingNodeService {
  ...
  public StreamingNode selectRandomNode(){
      ...
      Optional<StreamingNode> optionalStreamingNode = streamingNodeRepo.getStreamingNodeById(random_uuid);

       return optionalStreamingNode.orElseThrow(()->new ResourceNotFoundException("coud't find the vid  with the specifed id "+random_uuid)); // returning
  }
  ...
  public String getIpAndPortAddr(UUID nodId){
      ...
      Optional<StreamingNode> optionalRandStreamingNode = streamingNodeRepo.findById(
     ...
     StreamingNode selectedStreamingNode = optionalRandStreamingNode.orElseThrow(()-> new ResourceNotFoundException("" +
             "could't find the vid with the ID {}" + nodId));
  ...
  public void changeNodeStat(UUID nodeID){
      ...
      optionalStreamingNode.orElseThrow(()->new ResourceNotFoundException("there is no vid with the id"+ nodeID)).setUpStat(false);
  ...
  // so vid->reqeust->vid is still new -> then will pass it to the GOStremaingNodeService, and ask to give it random vid
