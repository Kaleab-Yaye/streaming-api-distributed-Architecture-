...
public class VidStoreService {
    ...
    public UUID getNodIdAssociatedWithVidId(UUID vidId){
        Optional<VidStoreLocation> optionalVidStoreLocation = vidStoreRepo.getVidStoreLocationByVidId(vidId);
        return  optionalVidStoreLocation.orElseThrow(()->new ResourceNotFoundException("could't find relation with the vid id of "+ vidId)).getStreamingNode().getId();
    }
